...
response_rewriting_prompt = """Generate title describe the main subject of the document:
[document]: {document}
Please output in JSON format as follows: {{"title": title}}."""
...
def process_item(document, filename):
    ...
    while attempt < retries:
        try:
            ...
            client = genai.Client(
                api_key="AIzaSyBkOcZyfctXNhakwwTrqEm-Nqh723sDC1I",
            )
            ...
            new_item = {}
            new_item["title"] = parser.parse(response.text)["title"]
            new_item["document_id"] = filename.split(".txt")[0]
            new_item["document"] = document
